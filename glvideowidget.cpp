...
void GLVideoWidget::renderYUV(uchar* ptr, long size, int width,int height,int type) {
    if(isRenderring) {
        setYUV420pParameters(width, height); //call once
        QMutexLocker lock(&m_mutex);
        Q_UNUSED(lock);
        upload_tex = true;
        m_yuvdata = ptr;
        plane[0].data = (char*)ptr;
        if (plane.size() > 1) {
            plane[1].data = plane[0].data + plane[0].stride*height;
            plane[2].data = plane[1].data + plane[1].stride*height/2;
        }
...
void GLVideoWidget::renderYUV(QByteArray& data,int width,int height,int type) {
    if(isRenderring) {
        setYUV420pParameters(width, height); //call once
        setFrameData(data);
    }
...
void GLVideoWidget::onRender(const QImage &image) {
    if(isRenderring) {
        QImage img = image.convertToFormat(QImage::Format_RGB888);
        setQImageParameters(img.format(), img.width(), img.height(), img.bytesPerLine()); //call once
        setImage(img);
...
void GLVideoWidget::startRenderring() {
    //isRenderring = true;
}
void GLVideoWidget::stopRenderring() {
    //isRenderring = false;
}
